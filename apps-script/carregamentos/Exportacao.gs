...
function buildFuncionariosMap_(abaFuncionarios) {
  ...
  for (var i = 1; i < dados.length; i++) {
    ...
    var nomeCompleto = idxNomeCompleto >= 0 ? String(linha[idxNomeCompleto] || '').trim() : '';
    var nomeFallback = idxNome >= 0 ? String(linha[idxNome] || '').trim() : '';
    mapa[codigo] = nomeCompleto || nomeFallback || codigo;
  }
  return mapa;
}
...
function buildLinhasExportacao_(abaRegistros, mapaItensPorRegistro, mapaFuncionarios) {
  ...
  for (var i = 1; i < dados.length; i++) {
    ...
    var tarefaLower = tarefa.toLowerCase();
    if (tarefaLower !== 'carregamento' && tarefaLower !== 'limpeza') continue;
    ...
    var qtdItens = tarefaLower === 'carregamento' ? (mapaItensPorRegistro[idRegistro] || 0) : 0;
